...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!question || !videoId) return NextResponse.json({ error: "question and videoId are required" }, { status: 400 });

    const store = vectorStores[videoId];
    if (!store) return NextResponse.json({ error: "Video not loaded. Please load the video first." }, { status: 400 });
    ...
    const similarities = store.embeddings.map((emb, idx) => ({
      idx, score: cosineSimilarity(questionEmbedding, emb),
    }));
